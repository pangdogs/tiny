...
// EventEntityManagerRemoveEntity 在实体进入 Leaving 且树关系已移除、但本地索引尚未删除时派发。
// +event-gen:export_emit=0
// +event-tab-gen:recursion=allow
...
// Tiny Runtime 仅在 Entity 处于 Awaking 至 Alive 时推进这些组件的生命周期。
// +event-gen:export_emit=0
// +event-tab-gen:recursion=allow
...
// Tiny Runtime 仅在 Entity 处于 Awaking 至 Alive 时推进组件的停用与销毁生命周期。
// +event-gen:export_emit=0
// +event-tab-gen:recursion=allow
...
// Tiny Runtime 仅在 Entity 处于 Awaking 至 Alive 时推进组件的启用或禁用生命周期。
// +event-gen:export_emit=0
// +event-tab-gen:recursion=allow
...
// Tiny Runtime 仅在 Entity 处于 Awaking 至 Alive 时处理该事件。
