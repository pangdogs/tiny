...
上下文、本地实体管理器、实体树、运行事件和可热插拔 add-in。任务队列与帧推进由
根包 tiny.Runtime 驱动。
...
使用根包 tiny.ContinueOn 把续体调度回运行协程。Delegate 与 DelegateVoid 变体仍然
可用。
...
用 NewContext 创建上下文后，再交给 tiny.NewRuntime 绑定和运行。同一个 Context 只能
绑定一次；Runtime 外部应使用其并发视图和 Submit/Post，不应直接访问实体局部状态。
*/
package runtime
