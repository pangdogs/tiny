...
func InjectRV(entity ec.Entity, target reflect.Value) error {
	if entity == nil {
		return fmt.Errorf("%w: %w: entity is nil", exception.ErrTiny, exception.ErrArgs)
	}
...
retry:
				...
				if ok {
					...
					if sep >= 0 {
						...
						if err := entity.AddComponent(prototype[sep+1:], comp); err != nil {
							return fmt.Errorf("%w: %w", exception.ErrTiny, err)
						}
	...
	case reflect.Pointer, reflect.Interface:
		if target.IsNil() {
			return fmt.Errorf("%w: target is nil", exception.ErrTiny)
		}
	...
	default:
		return fmt.Errorf("%w: invalid taget %s", exception.ErrTiny, targetRT.Kind())
	}
}
