...
// 该操作可能按当前 Runtime 已声明的原型创建并添加缺失组件。
...
retry:
		...
		for i := range target.NumField() {
			...
			if name != "" {
				...
				if comp != nil && comp.Reflected().Type().AssignableTo(field.Type) {
					setField(target.Field(i), field, comp.Reflected())
					continue
				}
			}
	...
	default:
		return fmt.Errorf("%w: invalid target %s", exception.ErrCore, targetRT.Kind())
	}
}

func setField(target reflect.Value, field reflect.StructField, value reflect.Value) {
	if field.IsExported() {
		target.Set(value)
		return
	}
