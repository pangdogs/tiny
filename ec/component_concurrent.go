...
// ConcurrentComponent 暴露可跨 goroutine 使用的组件身份、并发上下文和生命周期 Scope。
//
...
// AsyncScope 返回绑定组件生命周期的后台任务作用域，并在首次访问时懒创建。
