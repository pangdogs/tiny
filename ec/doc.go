...
/*
Package ec 提供 Tiny 的核心数据模型：Entity、Component、实体树节点状态、组件启停
事件与原型描述接口。
...
常见用法是：
  ...
  - 通过生命周期接口在 tiny.Runtime 驱动下接收 Awake/Start/Update/Shut/Dispose；
  - 通过 Entity 的组件管理与树节点接口组合复杂对象结构。

ec 包负责实体与组件本身的状态机与事件表，不负责原型库的声明与注册。原型系统
位于 ec/pt 包，运行时调度与生命周期推进则由根包 tiny 和 runtime 包完成。
...
为降低大量短生命周期对象的创建成本，Entity 和 Component 不预分配 Context、Signal
或 AsyncScope；对象级 Scope 在首次访问 AsyncScope 时创建，并在相应对象进入 Dead 后
关闭。对象销毁不会阻塞等待 Scope 中的任务退出，需要汇合时应在其他 goroutine 等待
Scope.Done。
*/
package ec
