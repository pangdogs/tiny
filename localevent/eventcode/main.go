...
//     `go:generate go run kit.golaxy.org/tiny/localevent/eventcode --decl_file=$GOFILE gen_eventtab --package=$GOPACKAGE --name=XXXEventTab`
//
...
func main() {
	...
	eventPackageAlias := kingpin.Flag("event_package_alias", "导入GOLAXY框架的`kit.golaxy.org/tiny/localevent`包时使用的别名。").Default("localevent").String()
